...
model.c = Param(model.M, model.N, initialize=1.0, within=Reals)
...
def cost_(model):
    return sum(model.d[j]*dist[j-1][i-1]*model.x[i,j] for i in model.M for j in model.N)
model.cost = Objective(sense=minimize, rule=cost_)
